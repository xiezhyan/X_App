package com.sanq.product.core.ui.edit;
...
public class PayPsdInputView extends AppCompatEditText {
    ...
    private void initPaint() {
        ...
        textPaint = getPaint(5, Paint.Style.FILL, getCurrentTextColor());
...
//       super.onDraw(canvas);
        ...
        if(getInputType() == InputType.TYPE_NUMBER_VARIATION_PASSWORD)
    ...
    private void drawText(Canvas canvas) {
        if(!TextUtils.isEmpty(mText)) {
            mText = mText.trim();
            LogUtils.d("text", mText.split("").length);
            for (int i = 0; i < textLength; i++) {
                canvas.drawText(mText.split("")[i + 1], (startX + i * 2 * startX) - (getTextSize()/4), startY + (getTextSize()/2), textPaint);
            }
        }
    }
